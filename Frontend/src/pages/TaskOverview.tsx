...
const TaskOverview: React.FC = () => {
    ...
    const refetchTasks = async () => {
        if (!token) return;
        try {
            const taskRes = await fetch(`${API_URL}/project_tasks/get-all-tasks`, {
                method: "POST",
            ...
            const overviewRes = await fetch(`${API_URL}/project_tasks/task-overview`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ token }),
            });
    ...
    useEffect(() => {
        ...
        const getOverview = async () => {
            try {
                const res = await fetch(`${API_URL}/project_tasks/task-overview`, {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ token }),
                });
        ...
        const getTasks = async () => {
            try {
                const res = await fetch(`${API_URL}/project_tasks/get-all-tasks`, {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ token }),
                });
        ...
        <div>
            <div className='flex justify-between'>
                ...
                <div className='flex flex-col gap-2'>
                    ...
                    <button onClick={() => setModal(7)} className='bg-blue-main text-sm rounded-md px-3 p-2 cursor-pointer'>Create new Task</button>
                    <button onClick={() => setModal(1)} className='bg-blue-main text-sm rounded-md px-3 p-2 cursor-pointer'>Create new hashtags</button>
                </div>
            </div>
                                        ...
                                        <tr key={index} className="border-b border-[#2C2C2C]">
                                            ...
                                            <td className="py-3 px-4  ">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    />
                                                    <p className="text-sm max-w-[200px] ml-2 text-white truncate">{item.description || item.hashtags}</p>
                                                </div>
                                            </td>
            ...
            <ModalWrapper onClose={() => setModal(0)} isOpen={modal === 1} >
                <Create_Task refresh={refetchTasks} type ={"hash"}  onClose={() => setModal(0)} />
            </ModalWrapper>

            <ModalWrapper onClose={() => setModal(5)} isOpen={modal === 7} >
                <Create_Task refresh={refetchTasks} type ={"task"} onClose={() => setModal(0)} />
            </ModalWrapper>
