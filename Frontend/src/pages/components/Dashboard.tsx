...
import No_Task from './No_Task';
import No_Ambassador from './No_Ambassador';
...
const Dashboard: React.FC = () => {
  ...
  // const image = localStorage.getItem("toke _image");
  ...
  useEffect(() => {
    ...
    const username = queryParams.get("handle");
    ...
    if (username) {
      localStorage.setItem("project_username", username);
    }
    if (image) {
      localStorage.setItem("project_image", image);
    }
    if (id) {
  ...
  useEffect(() => {
    const token = localStorage.getItem("project_token"); // Or however you store your admin token
    ...
    const fetchOverview = async () => {
      try {
        const res = await fetch(`${API_URL}/project_dashboard/get-overview`, {
          method: "POST",
    ...
    const fetchTopAmbassadors = async () => {
      ...
      try {
        const res = await fetch(`${API_URL}/project_dashboard/top-ambassadors`, {
          method: "POST",
          headers: {
    ...
    const fetchSubmittedTasks = async () => {
      ...
      try {
        const res = await fetch(`${API_URL}/project_dashboard/submitted-tasks`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ token }),
        });
    ...
    <div>
      ...
      <div className='lg:flex gap-6'>
        {
          dashboard.map((item, index) => (
            <div className='h-28 relative py-8 rounded-2xl w-full flex gap-4 items-center  bg-[#232323] my-6 px-4' key={index}>
              ...
              {
                index === 1 ? <p className='text-[#A28AE8] text-sm  absolute  right-4 cursor-pointer' onClick={() => navigate('/grindfi/home/task/overview')}>Create new task</p> : <></>
              }
            </div>
          ))
        }
      ...
      <div className='lg:flex'>
                        ...
                        <td className="py-2 px-4">
                          <span
                            className={`text-xs w- flex justify-center items-center px-3 py-2 rounded-md ${item.status === "approved"
                              ...
                              : "border-[#F17B2B] border  text-yellow-300"
                              }`}
                          >
        ...
        <div>
          ...
          {
            topAmbassadors.length === 0 && <No_Ambassador />
          }
          <div className='text-gray-400 l  text-xs '>
            {paginatedAmbassadors.map((board: Ambassador, index: number) => (
              <div
                ...
                key={`${board.ambassadorTwitter|| board.username}-${index}`}
              >
                <div className='flex lg:gap-8 items-center gap-2'>
                  ...
                  <img src={board.ambassadorImage} className='rounded-full mr-2 w-8 lg:w-12' alt='user' />

                  <div className='flex gap-2  flex-col items-start'>
                    <div className='flex gap-2 items-center'>
                      <p className='text-white truncate max-w-[50px] lg:text-[18px]'>{board.ambassadorName}</p>
                      <span>&bull;</span>
                      <a target='_blank' href={`https://x.com/${board.ambassadorTwitter}`} className='underline lg:text-[22px] truncate max-w-[50px] cursor-pointer'>@{board.ambassadorTwitter || board.username}</a>
                    </div>
                    <div className='flex lg:text-[18px] gap-4 items-center'>
                      <div className='flex items-center'>
                        <img src={score} alt='score' />
                        <p>Score : {board.totalPoints || 0}</p>
                      </div>
                      <div className='flex items-center'>
                        <img src={X} alt='tweet' />
                        <p>Tweets : {board.totalTasks || 0}</p>
                      </div>
                    </div>
                  </div>
                </div>
                <p className='text-white text-[12px] lg:text-[20px] ml-6 '>{board.points || 0} pts</p>
              </div>
            ))}
          </div>
          <Pagination
