...
import { adminpfp, arrow, home, logo, tasks } from "../../assets/index";
...
export default function Sidebar() {
  ...
  const [projectImg, setProjectImg] = useState(adminpfp);

  useEffect(() => {
    const storedImg = localStorage.getItem("project_image");
    if (storedImg) {
      setProjectImg(storedImg);
    }
  }, []);
  ...
  const SIDEBAR_LINKS = [
    ...
    },
    {
      ...
      href: "/grindfi/home/task",
      icon: tasks,
      children: [
        { label: "Tasks overview", href: "/grindfi/home/task/overview" },
        { label: "Completed Tasks", href: "/grindfi/home/task/submitted" },
      ],
    },
  ];
    ...
    <>
      ...
      <div className="lg:hidden w-[100vw] text-white flex py-1 justify-between items-center p-4">
        ...
        <div className="flex items-center">
          <img src={projectImg} className=" rounded-full w-12 h-12 mt-2" alt="" /> {/* 👈 replaced */}
      ...
      {/* Sidebar */}
      <aside
        className={`z-50 bg-[#161616] text-white fixed lg:static top-0 left-0 h-full w-[80%] lg:w-fit transition-transform duration-300 ${
              ...
              return (
                <div key={link.href} className="w-full">
                  <button
                    ...
                    } hover:bg-blue-main py-5 rounded-lg px-4 flex items-center gap-2 transition-all duration-300`}
                  >
                      ...
                      {link.children.map((child) => {
                        ...
                        return (
                          <Link
                            key={child.href}
                            to={child.href}
                            className={`text-sm px-2 py-1 rounded-md hover:bg-blue-main ${
                              isChildActive ? "bg-blue-main" : ""
                            }`}
